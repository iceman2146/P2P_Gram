...
void requestLoop(); // ќбрабатывает клиентские запросы, добавл€€ их в
                    // MessageQueue(producer)

void responseLoop(); // ќбрабатывает ответы сервера клиентам, считывает из
...
int main(int argc, char **argv) {
  ...
  boost::this_thread::sleep(boost::posix_time::millisec(int(sleepLen::sml)));
...
void requestLoop() {
  for (;;) {
    ...
    boost::this_thread::sleep(boost::posix_time::millisec(int(sleepLen::lon)));
  }
}
...
void disconnectClient(socket_ptr clientSock) {
  ...
  cout << "Client Disconnected! " << clientList->size() << " total clients"
       << endl;
}
